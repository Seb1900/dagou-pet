...
- [Mikutap](https://aidn.jp/mikutap/) 使用固定时间网格和预先校音的声库，使随意输入仍有稳定律动。大狗采用相同原则，但只使用项目自带的三个声音。
...
| 样本 | 实测锚点 | 8 档目标音 |
| --- | --- | --- |
| `da` | MIDI 71.32，约 B4 | F4、G4、A4、B4、C5、D5、E5、F5 |
| `gou` | MIDI 65.54，约 F♯4 | C4、D4、E4、F4、G4、A4、B4、C5 |
| `jiao` | MIDI 71.08，约 B4 | F4、G4、A4、B4、C5、D5、E5、F5 |

每个样本都保留一个偏差小于 1 个半音的近原声档，主声移调限制在约 `±7` 个半音以内。这个取舍无法替代 Mikutap 的 32 段独立录音，但能避免此前 `gou` 最高达到 `+8.46` 半音并同时叠加多个高增益副本所造成的失真。
